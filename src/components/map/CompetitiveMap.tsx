        ...
        content: `
          <div style="font-family:'DM Sans',sans-serif;padding:8px;min-width:180px">
            ...
            <div style="margin-top:8px;display:flex;gap:8px">
              <span style="background:#0d1b2a;color:#fff;padding:2px 8px;border-radius:100px;font-size:11px;font-weight:700">${overall_score.toFixed(1)} Score</span>
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'relative' }}>
        ...
        <div ref={mapRef} style={{ height: 380, display: loading || error ? 'none' : 'block' }} />
      </div>
      ...
      {mapData && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', borderTop: '1px solid #e2e8f0' }}>
          {[
            ...
            { label: 'Exceeding NQS nearby', value: mapData.stats.exceeding_nqs },
          ].map((s) => (
            <div key={s.label} style={{ padding: '10px 14px', borderRight: '1px solid #e2e8f0' }}>
              <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#5a7a94', marginBottom: 4 }}>
                {s.label}
              </div>
              <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 18, fontWeight: 500, color: s.color || '#0d1b2a' }}>
                {s.value}
              </div>
            </div>
          ))}
      ...
      {mapData && mapData.competitors.length > 0 && (
        <div style={{ borderTop: '1px solid #e2e8f0' }}>
          <div style={{ padding: '10px 16px', fontSize: 11, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#5a7a94', borderBottom: '1px solid #e2e8f0' }}>
            Nearby Centres
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f8fafc' }}>
                  {['Centre', 'Suburb', 'Places', 'NQS', 'Distance'].map(h => (
                    <th key={h} style={{ padding: '8px 12px', textAlign: 'left', fontSize: 11, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#5a7a94', borderBottom: '1px solid #e2e8f0' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {mapData.competitors.map((c) => (
                  ...
                  >
                    <td style={{ padding: '9px 12px', fontSize: 12, fontWeight: 600, color: '#0d1b2a', borderBottom: '1px solid #f1f5f9', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.name}</td>
                    <td style={{ padding: '9px 12px', fontSize: 12, color: '#5a7a94', borderBottom: '1px solid #f1f5f9' }}>{c.suburb}</td>
                    <td style={{ padding: '9px 12px', fontSize: 12, fontFamily: "'DM Mono', monospace", color: '#0d1b2a', borderBottom: '1px solid #f1f5f9' }}>{c.licensed_places || '—'}</td>
                    <td style={{ padding: '9px 12px', borderBottom: '1px solid #f1f5f9' }}>
                      <span style={{ fontSize: 11, fontWeight: 600, color: nqsColor(c.nqs_rating), background: `${nqsColor(c.nqs_rating)}18`, padding: '2px 8px', borderRadius: 100 }}>
                        {c.nqs_rating === 'Exceeding NQS' ? 'Exceeding' : c.nqs_rating === 'Working Towards NQS' ? 'Working Towards' : 'Meeting'}
                      </span>
                    </td>
                    <td style={{ padding: '9px 12px', fontSize: 12, fontFamily: "'DM Mono', monospace", color: '#5a7a94', borderBottom: '1px solid #f1f5f9' }}>
      ...
      <div style={{ display: 'flex', gap: 0, borderTop: '1px solid #e2e8f0' }}>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '8px 12px', fontSize: 11, color: '#5a7a94' }}>
          <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#0d1b2a', display: 'inline-block' }} /> Target
          <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#00b4a0', display: 'inline-block', marginLeft: 8 }} /> Competitor
        </div>
      </div>
    </div>
  )
