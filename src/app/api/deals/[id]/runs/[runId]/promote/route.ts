...
import { promoteUnderwritingRun } from '@/lib/underwritingRuns'
...
function migrationMessage(error: { message?: string } | null) {
  ...
  if (/underwriting_runs|current_run_id|relation .* does not exist|schema cache/i.test(error.message)) {
...
export async function POST(
  ...
  try {
    ...
    if (run.status !== 'completed') return NextResponse.json({ error: 'Only completed underwriting runs can be promoted' }, { status: 400 })
    if (!run.extracted || !run.scored || !run.workflow) {
      return NextResponse.json({ error: 'Run is missing extracted, scored, or workflow snapshots' }, { status: 400 })
    }

    const promotedRun = await promoteUnderwritingRun(supabaseAdmin, run as UnderwritingRun)
    return NextResponse.json({
      deal: { id, current_run_id: promotedRun.id },
      run: promotedRun,
    })
  } catch (e) {
    const message = e instanceof Error ? e.message : 'Failed to promote underwriting run'
