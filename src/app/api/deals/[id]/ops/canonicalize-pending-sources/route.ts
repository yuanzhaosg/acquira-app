...
import { canonicalizePendingSourceDocumentsForDeal } from '@/lib/dealOps'
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await canonicalizePendingSourceDocumentsForDeal(supabaseAdmin, {
      dealId: id,
      dryRun,
    })
    return NextResponse.json({ ...result, mutated: !dryRun })
  } catch (e) {
    const message = e instanceof Error ? e.message : 'Failed to canonicalize pending source documents'
