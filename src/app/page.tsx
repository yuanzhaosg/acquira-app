...
export default function Home() {
  ...
  // When user clicks upload on landing — gate behind auth
  ...
  // After successful auth, close modal — user can now navigate
  function handleAuthClose() {
    setShowAuth(false)
    // If user just signed in, take them to upload
    if (user) setView('upload')
  }

  // Don't flash landing before auth resolves
  if (loading) {
    return (
  ...
  // ── Sample report for unauthenticated users ───────────────────────────────
  if (view === 'sample') {
    return (
      <>
